...
export default function History() {
  ...
  const fetchAdjustments = async () => {
    try {
      const response = await axios.get("http://localhost:3001/adjustments");
      if (response.status === 200) {
  ...
  const handleTimeFilter = (filter: string) => {
    ...
    if (activeTab === "sales") {
      ...
      if (filter === "today") {
        ...
        filtered = rows.filter((row) => new Date(row.timestamp) >= oneWeekAgo);
      } else if (filter === "thisMonth") {
        filtered = rows.filter((row) => new Date(row.timestamp) >= oneMonthAgo);
      }

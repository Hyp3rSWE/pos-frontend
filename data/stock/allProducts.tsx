  ...
  export   const handleaddProduct = async (formRef:any,isGros:boolean) => {
    ...
    if(!isGros){
      ...
      try {
        ...
        //formData[]
        console.log("before")
        const response = await axios.get(`http://localhost:3001/products/barcode/${formData.get("product_barcode_for_variant")}`);
        console.log("after")
        console.log('Data:', response.data);
        //"product_id": 27,
        ...
        sendDataGros.variant_quantity = formData.get("product_quantity_for_variant")
        try {
